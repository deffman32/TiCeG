...
fat_ptr<uint8_t> serialize_decks(deck_t decks[MAX_DECKS]) {
  ...
  // Structure: For each deck [card_count(1) + name_length(1) + name(var) + cards(var)]
...
fat_ptr<deck_t> deserialize_decks(const fat_ptr<uint8_t> &bytes) {
  ...
  if (num_decks > MAX_DECKS) num_decks = MAX_DECKS;
  ...
  for (size_t i = 0; i < num_decks && offset < bytes.size; i++) {
    ...
    if (offset + name_length > bytes.size) break;
    ...
    if (card_count > 0) {
      ...
      if (offset + cards_size > bytes.size) break;
